...
export const Card = forwardRef<HTMLDivElement, CardType>(
  ({ headerText, children, isOpen, setIsOpen, icon, className, showArrow, ...props }: CardType, ref) => {
    ...
    return (
      <div className={s.card} ref={notificationRef}>
        <div onClick={() => setIsOpen(!isOpen)} ref={iconRef} className={s.iconContainer}>
          {showArrow &&<div className={s.arrow} style={isOpen ? styles.visible : styles.hidden}></div>}
          {icon}
        </div>
        <div
